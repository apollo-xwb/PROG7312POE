...
namespace MunicipalServicesApp.Managers
{
    public class EventManager
    {
        ...
        public async Task<SortedDictionary<DateTime, List<Event>>> GetAllEventsAsync()
        {
            var events = await _context.Events
                .OrderBy(e => e.Date)
                .ToListAsync();

            var eventsByDate = new SortedDictionary<DateTime, List<Event>>();
            foreach (var eventItem in events)
            {
                ...
                eventsByDate[date].Add(eventItem);
            }

            return eventsByDate;
        }

        public SortedDictionary<DateTime, List<Event>> GetAllEvents()
        {
        ...
        public async Task<List<Event>> SearchEventsAsync(string? keyword, string? category, DateTime? startDate, DateTime? endDate)
        {
        ...
        public List<Event> SearchEvents(string? keyword, string? category, DateTime? startDate, DateTime? endDate)
        {
            return SearchEventsAsync(keyword, category, startDate, endDate).Result;
        }

        public async Task<List<Event>> GetRecommendationsAsync()
        {
            return await _recommendationService.GetPersonalizedRecommendationsAsync();
        }
